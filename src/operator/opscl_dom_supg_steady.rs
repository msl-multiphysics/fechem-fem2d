...
impl OpSclDomSupgSteady {
    pub fn new(dom_id: usize, wgt_id: usize, tau_diff_id: usize, vel_id: usize, src_id: usize, unk_id: usize, diff_drv_ids: Vec<(usize, usize)>) -> OpSclDomSupgSteady {
        // adds steady SUPG stabilization to a scalar transport equation
        // d(m_i * c_i)/dt = -div(m_i * c_i * v - sum_j(D_ij * grad(c_j))) + R_i
        //
        ...
        // create struct
        let mut oper_supg = OpSclDomSupgSteady::default();
...
impl OperatorBase for OpSclDomSupgSteady {
    fn apply(&self, vars: &Variables, a_triplet: &mut Vec<Triplet<usize, usize, f64>>, b_vec: &mut Col<f64>, t: f64, factor: f64) {
        ...
        // tau * (v . grad(w), div(m * c * v) - div(sum_j(D_ij * grad(c_j))) - R_i)_dom
        ...
        for eid in 0..dom.num_elem {
            ...
            for qid in 0..num_quad {
                ...
                let [wgt_x, wgt_y] = wgt_scl.compute_quad_grad(vars, eid, qid, t);
                ...
                let div_wgt_vel = vel_x * wgt_x + vel_y * wgt_y + wgt * div_vel;
                ...
                for v in 0..num_node {
                    ...
                    for j in 0..num_node {
                        ...
                        let div_adv_j = wgt * vel_grad_j + div_wgt_vel * quad_n[qid][j];
