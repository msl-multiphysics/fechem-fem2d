...
#[derive(Default)]
pub struct OpSclBndOutflowUnity {
    // boundary
    pub bnd_id: usize,
...
impl OpSclBndOutflowUnity {
    pub fn new(bnd_id: usize, vel_id: usize, unk_id: usize) -> OpSclBndOutflowUnity {
        // applies an outflow BC to scalar transport equations
        // dc_i/dt = -div(N_i) + R_i
        // N_i += c_i * v
        // N_i . n = c_i * max(v . n, 0)   (outgoing only; zero conductive flux)
        //
        // vel - velocity vector (v)
        // unk - unknown scalar (c_i)
        // weight is unity (1)
        // n is the outward normal from the boundary
        // completes the boundary term omitted by OpSclDomAdvectionUnity
        ...
        // create struct
        let mut oper_out = OpSclBndOutflowUnity::default();
...
impl OperatorBase for OpSclBndOutflowUnity {
    fn apply(&self, vars: &Variables, a_triplet: &mut Vec<Triplet<usize, usize, f64>>, _b_vec: &mut Col<f64>, _t: f64, factor: f64) {
        ...
        for eid in 0..bnd.num_elem {
            ...
            for qid in 0..num_quad {
                ...
                let coeff = -factor * quad_w[qid] * vn_out;  // weight = 1
                for v in 0..num_node {
                    for j in 0..num_node {
