...
impl VectorDomain {
    pub fn new_from_constant(vecdom_id: usize, dom: &Domain, value_const_x: f64, value_const_y: f64, file_path: String) -> Result<VectorDomain, FEChemError> {
        ...
        vecdom.node_prev_x = vec![value_const_x; dom.num_node];
        vecdom.node_prev_y = vec![value_const_y; dom.num_node];
    ...
    pub fn new_from_function(vecdom_id: usize, dom: &Domain, value_func: Box<dyn Fn(f64, &[f64]) -> (f64, f64) + Send + Sync>, scldom_ids: Vec<usize>, file_path: String) -> Result<VectorDomain, FEChemError> {
        ...
        vecdom.node_prev_x = vec![0.0; dom.num_node];
        vecdom.node_prev_y = vec![0.0; dom.num_node];
        vecdom.node_dir = vec![false; dom.num_node];
        ...
        match num_node {
            3 => {
                ...
                for v in 0..num_node {
                    ...
                    val_x += n[v] * self.node_value_x[nid];
                    val_y += n[v] * self.node_value_y[nid];
                }
                return (val_x, val_y);
            }
            4 => {
    ...
    pub fn compute_quad_unknown_prev(&self, dom: &Domain, eid: usize, qid: usize) -> (f64, f64) {
        ...
        match num_node {
            3 => {
                ...
                for v in 0..num_node {
                    let nid = dom.elem_node_id[eid][v];
                    val_x += n[v] * self.node_prev_x[nid];
                    val_y += n[v] * self.node_prev_y[nid];
                }
                return (val_x, val_y);
            }
            4 => {
