...
pub struct SteadyHeatMassFlow {
    ...
    // operators
    pub oper_itr_heat: Vec<(OpSclDomDiffusion, OpSclDomSource)>,
    pub oper_itr_heat_adv: Vec<(OpSclDomAdvection, OpSclDomSupgSteady)>,
    ...
    pub oper_itr_mass_adv: Vec<(OpSclDomAdvectionUnity, OpSclDomSupgSteadyUnity)>,
    ...
    // operators
    pub oper_itr_flow: Vec<(OpVecDomAdvection, OpVecDomPressure, OpVecDomDiffusion, OpVecDomSource, OpVecDomSupgSteady, OpSclDomDivergence, OpSclDomPspgSteady)>,
...
impl SteadyHeatMassFlow {
    pub fn new(num_comp: usize) -> SteadyHeatMassFlow {
        // initialize struct
        let mut phys = SteadyHeatMassFlow::default();
...
impl SteadyBase for SteadyHeatMassFlow {
    ...
    fn initial_operator(&mut self, vars: &mut Variables) {
        ...
        for &dom_id in &self.itr_hdom {
            ...
            self.oper_itr_heat.push((oper_cond, oper_src));
            ...
            if self.itr_vel.contains_key(&dom_id) {
                ...
                self.oper_itr_heat_adv.push((oper_adv, oper_supg));
            }
        }
        ...
        for comp_i in 0..self.num_comp {
            for &dom_id in &self.itr_mdom {
                ...
                if self.itr_vel.contains_key(&dom_id) {
                    ...
                    self.oper_itr_mass_adv.push((oper_adv, oper_supg));
                }
            }
        ...
        for &dom_id in &self.itr_fdom {
            ...
            self.oper_itr_flow.push((oper_adv, oper_pres, oper_diff, oper_src, oper_supg, oper_div, oper_pspg));
        }
    ...
    fn assemble_matrix(&self, vars: &Variables, a_triplet: &mut Vec<Triplet<usize, usize, f64>>, b_vec: &mut Col<f64>) {
        ...
        for (oper_adv, oper_pres, oper_diff, oper_src, oper_supg, oper_div, oper_pspg) in &self.oper_itr_flow {
            oper_adv.apply(vars, a_triplet, b_vec, 0.0, 1.0);
            oper_pres.apply(vars, a_triplet, b_vec, 0.0, 1.0);
            oper_diff.apply(vars, a_triplet, b_vec, 0.0, 1.0);
            oper_src.apply(vars, a_triplet, b_vec, 0.0, 1.0);
            oper_supg.apply(vars, a_triplet, b_vec, 0.0, 1.0);
            oper_div.apply(vars, a_triplet, b_vec, 0.0, 1.0);
            oper_pspg.apply(vars, a_triplet, b_vec, 0.0, 1.0);
        }

        // assemble heat internal data
        for (oper_cond, oper_src) in &self.oper_itr_heat {
            oper_cond.apply(vars, a_triplet, b_vec, 0.0, 1.0);
            oper_src.apply(vars, a_triplet, b_vec, 0.0, 1.0);
        }
        for (oper_adv, oper_supg) in &self.oper_itr_heat_adv {
            oper_adv.apply(vars, a_triplet, b_vec, 0.0, 1.0);
            oper_supg.apply(vars, a_triplet, b_vec, 0.0, 1.0);
        }
        ...
        for (oper_adv, oper_supg) in &self.oper_itr_mass_adv {
            oper_adv.apply(vars, a_triplet, b_vec, 0.0, 1.0);
            oper_supg.apply(vars, a_triplet, b_vec, 0.0, 1.0);
        }
        ...
        for oper_htrn in &self.oper_bnd_htrn {
            oper_htrn.apply(vars, a_triplet, b_vec, 0.0, 1.0);
        }
        for oper_out in &self.oper_bnd_hout {
            oper_out.apply(vars, a_triplet, b_vec, 0.0, 1.0);
        }
        ...
        for oper_hres in &self.oper_hres_itf {
            oper_hres.apply(vars, a_triplet, b_vec, 0.0, 1.0);
        }
