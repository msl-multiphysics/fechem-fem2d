...
fn main() -> Result<(), FEChemError> {
    ...
    let vlcp = vars.add_scldom_con(dom, 2.0 * 5.0, "".to_string())?;  // volumetric heat capacity
    ...
    phys.add_heat_dom(dom, temp, vlcp, cond, hsrc);
    ...
    phys.solve(&mut vars, Box::new(solver), 0.1, 100, 1, 10, 1e-3,  1.0)?;
