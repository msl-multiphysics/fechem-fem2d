...
/// Steady Navier-Stokes equation though a channel
...
///
/// Boundary conditions:
/// - Left boundary (velocity): <0.001, 0.0> m s-1
...
fn main() -> Result<(), FEChemError> {
    ...
    let mut vars = Variables::new_from_bounds(0.0, 0.0, 1.0, 0.2, 50, 10)?;
    ...
    phys.solve(&mut vars, Box::new(solver), 100, 1e-3, 1.0)?;
