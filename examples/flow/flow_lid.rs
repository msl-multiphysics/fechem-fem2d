...
///
/// Geometry:
/// - Square with smaller tri elements near the walls
///
/// Properties:
...
///
/// Boundary conditions:
/// - Left boundary (velocity): <0.0, 0.0> m s-1
/// - Bottom boundary (velocity): <0.0, 0.0> m s-1
/// - Right boundary (velocity): <0.0, 0.0> m s-1
/// - Top boundary (velocity): <4.0e-4, 0.0> m s-1
...
fn main() -> Result<(), FEChemError> {
    ...
    let visc = vars.add_scldom_con(dom, 0.001, "".to_string())?;
    ...
    let v_t = vars.add_vecbnd_con(bnd_t, 4.0e-4, 0.0, "".to_string())?;
    ...
    phys.set_pres_ref(dom, 0, 0.0);  // p = 0 at point index 0 (arbitrary)
